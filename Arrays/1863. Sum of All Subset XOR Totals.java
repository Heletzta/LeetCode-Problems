...
class Solution {
    public int subsetXORSum(int[] nums) {
        // Use depth first search to explore all possible subsets and calculate the XOR for each
        ...
        for (int i = 1; i < subsets.size(); i++) {
            xor = subsets.get(i).get(0);
            for (int j = 1; j < subsets.get(i).size(); j++) {
                xor = xor ^ subsets.get(i).get(j);
            }
    ...
    // from geeks for geeks -- subset finder
    //Changed it from printing to saving the subsets
